...
- Los errores "no encontrado"/"ya existe" de un dominio migrado siguen usando los sentinels compartidos `db.ErrNotFound`/`db.ErrConflict` (no uno propio por dominio) — es vocabulario que ya usan `handlers/respond.go` y el resto de los handlers vía `errors.Is`, y mantenerlo evita tocar esa capa solo por mover un dominio de paquete.
- Primeros dominios construidos así: `enrollment` y `certificate` (nuevos). `course`, `lesson`, `user` y `authtoken` son los dominios **preexistentes** migrados a este patrón — cada uno se movió tal cual (mismo SQL, sin cambios de comportamiento) a `api/internal/<dominio>/`, con una excepción real en `user`: las dos reglas de "cambio de rol" (no auto-cambiarse el rol, no degradar al último admin), antes partidas entre `handlers/admin.go` y `db/users.go`, se consolidaron en `userapp.Service.UpdateRole` — así un caller que llame al service directo (no solo el handler HTTP) queda protegido igual, sin tener que repetir el chequeo.
- El paquete `api/auth/` (JWT, cookies, bcrypt, OAuth) **no se migra, a propósito**: son funciones puras/wrappers sobre librerías externas, sin pool de DB ni forma de repositorio — forzarlo a domain/application/infrastructure sería sobre-ingeniería sin nada real que separar. Con `course`/`lesson`/`user`/`authtoken` migrados, la migración incremental de dominios preexistentes queda completa.
